...
public class LightCaster : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector3[] objverts = sceneMeshFilters[0].mesh.vertices;
        for (int i = 1; i < sceneMeshFilters.Count; i++)
        {

            objverts = ConcatArrays(objverts, sceneMeshFilters[i].mesh.vertices);
        }
        ...
        for (int j = 0; j < sceneObjects.Length; j++) //cycle through all scene objects.
        {
            for (Int64 i = 0; i < sceneMeshFilters[j].mesh.vertices.Length; i++) //cycle through all vertices in the current scene object.
            {
                ...
                Physics.Raycast(transform.position, new Vector2((other.x - me.x) - offset, (other.y - me.y) - offset), out hit, 10000);
                RaycastHit hit2;
                Physics.Raycast(transform.position, new Vector2((other.x - me.x) + offset, (other.y - me.y) + offset), out hit2, 10000);
        ...
        Vector3 me2 = this.transform.position;// just to make the current position shorter to reference.
        Vector3 panel1V3 = panel1.transform.position;
        Vector3 panel2V3 = panel2.transform.position;

        RaycastHit hitPanel; //create and fire the two rays from the center of the light source in the direction of the vertex, with offsets.
        Physics.Raycast(transform.position, new Vector2((panel1V3.x - me2.x), (panel1V3.y - me2.y)), out hitPanel, 10000);
        RaycastHit hitPanel2;
        Physics.Raycast(transform.position, new Vector2((panel2V3.x - me2.x), (panel2V3.y - me2.y)), out hitPanel2, 10000);

        //if (hitPanel.collider != null)//for debugging: draw the rays cast.
        //{
        //    Debug.DrawLine(transform.position, hitPanel.point, Color.red);
        //}
        //if (hitPanel2.collider != null)
        //{
        //    Debug.DrawLine(transform.position, hitPanel2.point, Color.green);
        //}

        bool iluminado = (hitPanel.collider != null && hitPanel.collider.gameObject.name == panel1.gameObject.name) || (hitPanel2.collider != null && hitPanel2.collider.gameObject.name == panel2.gameObject.name);
        ...
        for (Int64 i = 0; i < angleds.Length; i++)//store the values in the struct array in verts and uvs. 
        ...
        for (Int64 i = 0; i < uvs.Length; i++)//offset all the UVs by .5 on both s and t to make the texture center be at the object center.
        {
